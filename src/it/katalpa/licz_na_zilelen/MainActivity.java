...
public class MainActivity extends FragmentActivity implements
	...
	void initApp() {
		...
		// "("+myPosition+"),("+map.getCameraPosition().target.latitude+","+map.getCameraPosition().target.latitude+")",
	...
	public void refreshMap(int iCount) {
		...
		List<PleaceObject> copy = new ArrayList<PleaceObject>(nearObjects); // shallow
																			// copy
		...
		// Toast.makeText(getApplicationContext(),
		// "Odśwież:"+map.getCameraPosition().target.latitude+","+map.getCameraPosition().target.longitude,
		// Toast.LENGTH_LONG);
	}
	...
	void ShowObjectDialog(final PleaceObject obj) {
		...
		// Toast.makeText(getApplicationContext(),
		// "Dialog:"+map.getCameraPosition().target.latitude+","+map.getCameraPosition().target.longitude,
		// Toast.LENGTH_LONG);
		...
		System.out.println("!!!!!!!!!Dialog2:"
				...
				+ map.getCameraPosition().target.longitude);
		// Toast.makeText(this,
		// "Dialog2:"+map.getCameraPosition().target.latitude+","+map.getCameraPosition().target.longitude,
		// Toast.LENGTH_LONG);
	...
	private void executeGeoSerach(final RelativeLayout mNearDialogView) {
		...
		try {
			...
			if (textLocation != null) {
				...
				addresses = geoCoder.getFromLocationName(
						((EditText) mNearDialogView
								...
								.toString(), 1, reg[0], reg[3], reg[1], reg[2]); 
				System.out.println("("+reg[0]+","+reg[3]+";"+reg[1]+","+reg[2]+")");
				System.out.println("-------");
				...
				addresses = geoCoder.getFromLocationName(
						((EditText) mNearDialogView
								.findViewById(R.id.textEditDialog)).getText() 
								.toString() + ","+textLocation + " Polska", 1); 
				System.out.println("-------");
				System.out.println(addresses);												
			//}
			...
			if (addresses != null && addresses.size() > 0) {
				 ...
				 * addresses.get(0).getLongitude() )); CameraUpdate zoom =
			...
			builder.setMessage("Niestety nie odnaleziono lokalizacji dla tego miejsca").setTitle("Geolokalizacja");
