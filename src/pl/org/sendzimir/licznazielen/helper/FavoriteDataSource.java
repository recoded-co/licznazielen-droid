...
public class FavoriteDataSource {
	...
	private String[] allColumns = { DatabaseHelper.KEY_ID,
			DatabaseHelper.COLUMN_FAV_OBJID, DatabaseHelper.COLUMN_FAV_NAME,
			...
			DatabaseHelper.COLUMN_FAV_ICONS, DatabaseHelper.COLUMN_FAV_ANSWRWS };
		...
		try {
			if (obj.getComments() != null)
				for (ComentsMap nm : obj.getComments()) {
					array.put(new JSONObject().put("name", nm.getKey()).put(
							"value", nm.getValue()));
				}
	...
	public void deleteFavoriteByObjId(int id) {
		...
		database.delete(DatabaseHelper.TABLE_NAME_FAV,
				DatabaseHelper.COLUMN_FAV_OBJID + " = " + id, null);
	}
	...
	public PleaceObject getFavoriteByObjId(int iId) {
		...
		Cursor cursor = database.query(DatabaseHelper.TABLE_NAME_FAV,
				allColumns, DatabaseHelper.COLUMN_FAV_OBJID + "=?",
				new String[] { "" + iId }, null, null, null, null);
	...
	private PleaceObject cursorToPleaceObject(Cursor cursor) {
		...
		try {
			...
			for (int i = 0; i < array.length(); i++) {
				...
				ComentsMap okl = obj.new ComentsMap(array.getJSONObject(i)
						.getString("name"), array.getJSONObject(i).getString(
						"value"));
